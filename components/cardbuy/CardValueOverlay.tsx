...
export function CardValueOverlay({ offerGbp, belowMin }: Props) {
  if (belowMin) {
    return (
      ...
      >
        <div className="relative -rotate-[8deg] border-[3px] border-warn rounded-sm bg-sun/85 px-2.5 py-1.5 shadow-[3px_3px_0_0_rgba(0,0,0,0.18)]">
  ...
  return (
    <span className="absolute bottom-1 right-1 pop-card rounded-sm bg-sun px-2 py-0.5 font-display text-[11px] tracking-wider tabular-nums text-ink z-10">
      {formatGBP(offerGbp)}
    </span>
  );
}
