...
export default async function BinderPage({
  ...
  const allSlots: BinderSlotPayload[] = NATIONAL_DEX.map((dex) => {
    ...
    // Species silhouette — the vendored black-shadow PNG generated by
    // scripts/build-silhouettes.mjs from PokeAPI official artwork.
    // Used by the missing-slot grid tile (not the preview pane).
    const speciesSilhouette =
      dex.number >= 1 && dex.number <= 1025
        ? `/silhouettes/${dex.number}.png`
        : null;

    // Species artwork — the coloured 240px Sugimori PNG vendored by
    // scripts/build-pokemon-artwork.mjs. Used by the OWNED grid tile
    // in place of the user's specific card art, so the binder reads
    // as a Pokédex (species-first) rather than a card binder.
    const speciesArtwork =
      dex.number >= 1 && dex.number <= 1025
        ? `/pokemon-artwork/${dex.number}.png`
        : null;
    ...
    const ownedEntries: BinderOwnedEntry[] = entries.map((e) => {
      ...
      return {
        ...
        cardId: e.card_id,
        // Per-entry card detail for the owned-card carousel — every
        // entry carries enough to drive its own slide hero + meta
        // panel + clickable set logo.
        card: card ? toBinderEntryCard(card) : undefined,
      };
    });
    ...
    return {
      ...base,
      owned: {
        // Headline card = first entry's print, kept on `owned.card`
        // for back-compat (the locked preview pane reads from this
        // until the carousel pulls per-entry).
        card: toBinderEntryCard(rep),
...
/** Project a fixture `Card` onto the slimmer `BinderEntryCard` shape
 *  the binder UI consumes. Resolves the set's logo URL via the shared
 *  fixtures helper so the carousel can render a clickable set chip
 *  per slide without duplicating set-lookup logic on the client. */
function toBinderEntryCard(card: Card): BinderEntryCard {
